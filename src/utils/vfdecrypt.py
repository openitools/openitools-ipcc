...
class V1Header:
    def __init__(self, data: bytes):
        ...
        la, = struct.unpack(">I", data[0x88:0x8C]); self.len_wrapped_aes_key = la
        self.wrapped_aes_key      = data[0x8C:0x8C+la]
        lh, = struct.unpack(">I", data[0x1B4:0x1B8]); self.len_hmac_sha1_key = lh
...
class V2Header:
    def __init__(self, data: bytes):
        ...
        off += 16  # skip UUID
        self.blocksize, = struct.unpack(">I", data[off:off+4]); off += 4
        self.datasize,  = struct.unpack(">Q", data[off:off+8]); off += 8
        self.dataoffset,= struct.unpack(">Q", data[off:off+8]); off += 8
        off += 0x260
        _,_,ki,ks = struct.unpack(">4I", data[off:off+16]); off += 16
        self.kdf_iteration_count, self.kdf_salt_len = ki, ks
        self.kdf_salt = data[off:off+ks]; off += ks
        be, = struct.unpack(">I", data[off:off+4]); off += 4
        self.blob_enc_iv = data[off:off+be]; off += be
        (self.blob_enc_key_bits,
         self.blob_enc_algorithm,
         self.blob_enc_padding,
         self.blob_enc_mode) = struct.unpack(">4I", data[off:off+16]); off += 16
        eb, = struct.unpack(">I", data[off:off+4]); off += 4
        self.encrypted_keyblob = data[off:off+eb]

def _determine_version(path: Path) -> int:
    with path.open("rb") as f:
        sig = f.read(8)
        if sig == b"encrcdsa": return 2
        f.seek(-8, 2)
        if f.read(8) == b"cdsaencr": return 1
    raise ValueError("Unknown header format")

def _precompute_chunk_ivs(hmac_key: bytes, max_chunks: int) -> list[bytes]:
    return [
        hmac.new(hmac_key, i.to_bytes(4, "big"), hashlib.sha1)
            .digest()[:CIPHER_BLOCKSIZE]
...
def decrypt_vf(path_in: Path, path_out: Path, key: str):
    ...
    _decrypt_image(path_in, path_out, aes_key, hmac_key, chunk_size, data_offset, data_size)
