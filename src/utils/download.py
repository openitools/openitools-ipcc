...
async def get_response(
    ...
    if remote_file_size is not None and file_size >= remote_file_size:
        logger.warning(
            f"Local file bigger or equal than server file, local: {file_size}, server: {remote_file_size}"
        )
    ...
    if file_size > 0:
        ...
        logger.info(
            f"resuming download from byte {file_size}, remote file size: {remote_file_size}"
        )
